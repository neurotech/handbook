	...
	return (
		<DialogPortal>
			...
			<DialogPrimitive.Popup
				...
				className={cn(
					"data-open:fade-in-0 data-open:zoom-in-95 data-closed:fade-out-0 data-closed:zoom-out-95 fixed top-1/2 left-1/2 z-50 flex max-h-[min(90dvh,calc(100dvh-env(safe-area-inset-top)-env(safe-area-inset-bottom)-1.5rem))] w-full max-w-[calc(100vw-2rem)] -translate-x-1/2 -translate-y-1/2 flex-col gap-4 overflow-y-auto rounded-xl bg-popover p-4 text-popover-foreground text-sm outline-none ring-1 ring-foreground/10 duration-100 data-closed:animate-out data-open:animate-in sm:max-w-sm",
					className,
				)}
				{...props}
			>
...
function DialogFooter({
	...
	return (
		<div
			...
			className={cn(
				"flex flex-col-reverse gap-2 rounded-b-xl border-border border-t bg-muted/50 py-4 sm:flex-row sm:justify-end",
				className,
			)}
			{...props}
		>
